					...
					<form:form id="myForm" method="post"
						...
						<fieldset>
									...
									<div class="col-lg-9">
										<select id="customerInput" name="user.id" class="form-control">  
         								   <c:forEach var="user" items="${users}">
							                 <option <c:if test="${user.id eq reserve.user.id}">selected="selected"</c:if> value="${user.id}">${user.name}</option>
							               </c:forEach>
							            </select>
									</div>
							</div>
									...
									<div class="col-lg-9">
										<select id="comicInput" name="comic.id" class="form-control">  
         								   <c:forEach var="comic" items="${comics}">
							                  <option <c:if test="${comic.id eq reserve.comic.id}">selected="selected"</c:if> value="${comic.id}">${comic.title}</option>
							               </c:forEach>
							            </select>
									</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-lg-9">
									<label class="radio-inline"><form:radiobutton path="status" value="0"  />In attesa</label>
									<label class="radio-inline"><form:radiobutton path="status" value="1"  />Disponibile</label>
							...
							<div class="form-group">
								...
								<form:input type="numeric" class="form-control" path="qty"
										id="qtyInput" placeholder="Qty" required="required" value="${reserve.qty}" onBlur="calculateTotal()"/>
